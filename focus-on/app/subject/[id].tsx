...
const DIFFICULTIES = [
  { id: 1, label: 'খুব সহজ', color: '#10B981', minutes: 15 },
  { id: 2, label: 'সহজ',     color: '#6C63FF', minutes: 25 },
  { id: 3, label: 'মাঝারি',  color: '#F59E0B', minutes: 35 },
  { id: 4, label: 'কঠিন',    color: '#EF4444', minutes: 50 },
  { id: 5, label: 'খুব কঠিন',color: '#DC2626', minutes: 60 },
];
...
export default function SubjectDetailScreen() {
  ...
  const deleteChapter = (chId: string) => {
    Alert.alert('Chapter delete?', 'এই chapter ও সব topics মুছে যাবে।', [
    ...
    <View style={[styles.root, { backgroundColor: c.bg }]}>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        {subject.chapters.length === 0 ? (
          <View style={styles.empty}>
            ...
            <Text style={[styles.emptyTxt, { color: c.textMuted }]}>Chapter যোগ করো উপরের + বাটনে</Text>
          </View>
        ) : (
                ...
                {chapter.topics.length === 0 ? (
                  !subject.topicBased && (
                    <TouchableOpacity style={[styles.emptyTopics, { borderColor: c.border }]}
                      ...
                      <Text style={[styles.emptyTopicsTxt, { color: c.textFaint }]}>Topic যোগ করো</Text>
                    </TouchableOpacity>
                  )
                ) : (
      ...
      <Modal visible={showAddChapter} transparent animationType="slide" onRequestClose={() => setShowAddChapter(false)}>
        <Pressable style={styles.modalBg} onPress={() => setShowAddChapter(false)}>
          <Pressable style={[styles.sheet, { backgroundColor: c.bgCard }]} onPress={e => e.stopPropagation()}>
            ...
            <Text style={[styles.sheetTitle, { color: c.text }]}>নতুন Chapter</Text>
            <TextInput
              style={[styles.input, { backgroundColor: c.inputBg, color: c.text, borderColor: c.border }]}
              placeholder="Chapter এর নাম..." placeholderTextColor={c.textFaint}
            ...
            <TouchableOpacity style={[styles.createBtn, { backgroundColor: subject.color, opacity: chapterName.trim() ? 1 : 0.5 }]}
              ...
              <Text style={styles.createTxt}>যোগ করো</Text>
            </TouchableOpacity>
          </Pressable>
        </Pressable>
      </Modal>
      ...
      <Modal visible={!!addingTopicForChapter} transparent animationType="slide" onRequestClose={() => setAddingTopicForChapter(null)}>
        <Pressable style={styles.modalBg} onPress={() => setAddingTopicForChapter(null)}>
          <Pressable style={[styles.sheet, { backgroundColor: c.bgCard }]} onPress={e => e.stopPropagation()}>
            <View style={[styles.handle, { backgroundColor: c.border }]} />
            <Text style={[styles.sheetTitle, { color: c.text }]}>নতুন Topic</Text>
            <TextInput
              style={[styles.input, { backgroundColor: c.inputBg, color: c.text, borderColor: c.border }]}
              placeholder="Topic এর নাম..." placeholderTextColor={c.textFaint}
...
const styles = StyleSheet.create({
  ...
  chapterName: { fontSize: 15, fontWeight: '700', flex: 1 },
  ...
  chAddTxt: { fontSize: 12, fontWeight: '700' },
  delBtn: { padding: 4 },
  ...
  sLabel: { fontSize: 12, fontWeight: '700', textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: 10 },
  ...
  diffOptTxt: { fontSize: 12, fontWeight: '700' },
